...
| --------------- | ---------------------------------------- |
| `inventories/`  | Environment-specific inventories (`oci`, `prod`) |
...
- `inventories/oci/hosts.ini`
- `inventories/prod/hosts.ini`

Environment mapping:
- `oci` -> OCI VM cluster
- `prod` -> homelab cluster on Proxmox
...
```bash
ansible-playbook -i inventories/oci/hosts.ini main.yaml
